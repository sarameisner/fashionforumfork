...
export async function getServerSideProps({ params }) {
  ...
  const { data, error } = await supabase
    .from('fashionforum')
    .select('id, image, dato, content, forfatter, fotograf, tags, categori, overskrift')
    .eq('id', id)
    .single();
...
export default function ArtikelSide({ artikel }) {
  ...
  <Image
        ...
        width={20}
        height={20}
        className="transition-transform duration-200 rotate-[180deg]"
      />
   <Link href="/"><button className="flex cursor-pointer hover:underline">
      <p>Tilbage</p>
  ...
  <p>{artikel.dato}</p>
    <p >
      ...
      {artikel.categori && <> <span className="font-medium">{artikel.categori}</span></>}
      {artikel.tags ? (
        <> - <span >{artikel.tags}</span></> ) : null}
